...
ca_childcare |>
    ...
    mc_infant == min(mc_infant, na.rm = TRUE)
    ) |>
    select(region, 
            mc_infant, 
            mhi_2018) |> 
    filter(mhi_2018 == min(mhi_2018, na.rm = TRUE)
           ) |>
    select(region, mhi_2018)
```
...
```{r}
#| label: dvs-2-num-cat
untitled_pink <- c("F" = "#e75480",
                  "M" = "#6a5acd") 
ggplot(surveys, aes(x = sex, y = weight, fill = sex)) + 
    geom_boxplot(outlier.shape = NA) +
    scale_fill_manual(values = untitled_pink) + 
    labs(
      title = "Relationship between Sex and Weight", 
      x = "Sex", 
      y = "Weight (g)") 
```
...
```{r}
#| label: dsm-2-2
chisq_table <- table(teacher_evals_compare$set_level, teacher_evals_compare$sen_level)
chisq_table
